...
class MyTCPHandler(socketserver.BaseRequestHandler):
    ...
    def instruction_distributor(self, instructions):
        ...
        print("in1",instructions)#cd|{"cwd": ["", "animal"]}
        instructions = instructions.split('|')
        print("in2",instructions)#['cd', '{"cwd": ["", "animal"]}']
    ...
    def dir(self, user_data):
        ...
        self.request.send(cmd_result)
        #self.request.send(json.dumps({"result": cmd_result}).encode())

    def cd(self, user_data):
        json_data = json.loads(user_data)# {'cwd': ['', 'animal']}

        new_dir = json_data['cwd']#['', 'animal']
    ...
    def file_put(self, user_data):
        ...
        if os.path.exists(file_abs_path):
            self.request.sendall(bytes("2003","utf8"))
            ...
            self.request.sendall(bytes("2002","utf8"))
            f=open(file_abs_path,"wb")
    ...
    def get_file_abs_path(self, filename):
        return "%s/%s%s%s" % (settings.USER_HOME, self.login_user.username, \
                               '/'.join(self.login_user.cwd) + '/', filename)
    ...
    def file_get(self, user_data):
        ...
        if os.path.isfile(file_abs_path):
            ...
            client_response_json = json.loads(self.request.recv(104).decode())
